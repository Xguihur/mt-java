...
```
1. Redis SADD rp:{id}:grabbed userId
   → 返回 0：已抢过，直接拦截
   → 返回 1：首次抢，继续

2. Redis LPOP rp:{id}:amounts
   → 返回 null：红包已抢完，撤回 SADD，抛出提示
   → 返回金额字符串：成功抢到
...
`LPOP` 是 Redis 原子操作，同一时刻只有一个请求能弹出同一个元素，  
所以即使 1000 个请求同时抢 10 个红包，也只有 10 个人能成功。
